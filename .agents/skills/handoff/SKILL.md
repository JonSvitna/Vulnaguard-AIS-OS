...
Manual agent switching, made seamless. Sean codes across Codex, Codex CLI, and Cursor. When he hits a limit on one, this skill writes down exactly what's in flight so whichever agent picks it up next doesn't have to re-derive context.

Two things make this work:

1. **One operating manual, three formats.** `AGENTS.md` is canonical. `AGENTS.md` (Codex) and `.cursor/rules/aios.mdc` (Cursor) are generated copies — same content, different filename each tool expects natively.
...
Run this every time `AGENTS.md` changed since the last handoff — keeps Codex/Cursor's view of the AIOS from drifting out of sync. Cheap to run even if nothing changed.
...
- If `AGENTS.md` changes and this skill isn't run, `AGENTS.md` / `.cursor/rules/aios.mdc` drift stale. There's no hook auto-running the sync — re-run Step 1 whenever the manual changes, not just at handoff time.
