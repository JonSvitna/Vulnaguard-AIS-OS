...
## 2026-06-19 — Spun up `video-website-agent` as a new repo, separate from this AIOS

**Decision:** Created a new private GitHub repo (`github.com/JonSvitna/video-website-agent`) with two sibling folders: `video/` (a HyperFrames composition workspace, scaffolded via `npx hyperframes init` + `npx skills add heygen-com/hyperframes`, with 26 requested content blocks installed — caption styles, VFX, social overlays, app showcase, data/map charts, parallax, morph text, terminal/code snippets) and `website/` (placeholder only, no code yet). This repo is separate from `Vulnaguard-AIS-OS` — it's a product-line repo like Sentinel CMMC or the SEO agent, not part of the AIOS's own structure.

**Why:** Sean wants to combine the planned video-editing feature (previously scoped under the SEO agent in `CLAUDE.md`) with a new website-creation agent into one product line, and asked to scaffold it now — explicitly "so we can reel this in a bit, I don't want this becoming the wild wild west with no structure" — rather than letting HyperFrames blocks or website-agent code land ad-hoc in whichever repo was open. Splitting `video/` and `website/` as siblings keeps each half's tooling (HyperFrames composition files vs. whatever the website agent ends up using) from tangling together while sharing one repo/home.

**Alternatives considered:** Installing the HyperFrames blocks into `vulnaguard-seo-agent` (rejected — that repo's planned video feature was scoped narrower than this, and a website-creation agent doesn't belong there either); splitting video and website into two separate repos (rejected by Sean — wants one combined repo/name, `video-website-agent`); installing as a global Claude Code skill (not applicable — `hyperframes add` writes project files, not skills, into a target directory).

**Owner:** Sean. Open: `website/` half is just a placeholder — no scope, no code. `video/` has tooling but no actual compositions built yet. Revisit `connections.md` / `context/priorities.md` if this becomes a real active line alongside Sentinel CMMC and the SEO agent.

## 2026-06-19 — Connected Obsidian as the AIOS's second-brain layer via claude-obsidian

**Decision:** Installed the `claude-obsidian` Claude Code plugin (AgriciDaniel/claude-obsidian, MIT, 7.1k stars) at user scope and ran its `/wiki` setup against Sean's existing vault at `~/Documents/Obsidian Vault` (not a fresh clone, to keep his existing notes). It scaffolded a domain-first wiki (`wiki/domains/sentinel-cmmc`, `wiki/domains/seo-agent`, `wiki/domains/client-work`) matching the same three lines of work this AIOS's `CLAUDE.md` already tracks. Then ran the `wiki-ingest` skill on Sean's existing loose notes (`Claude brain/`, `ICP/` — 7 files) via 7 parallel sub-agents, producing 7 source pages, 6 entity pages, and 8 concept pages, cross-linked into the domains. Originals left untouched.

**Why:** Sean wanted a way to organize knowledge with wikis and prevent redundancy, prompted by a Nate Herk video on the Four Cs framework (the same framework this AIOS's `/audit` skill already uses) and the AgriciDaniel/claude-obsidian repo. The plugin's domain-first wiki gives a structured, cross-linked second brain that complements (doesn't replace) this AIOS's existing memory/decisions/references system — the AIOS stays the operating layer (skills, cadence, connections); the Obsidian vault becomes the deeper knowledge layer (entities, concepts, source provenance) that this AIOS and other Claude Code projects can point at instead of duplicating context.

**Alternatives considered:** Using `/graphify` (already installed) for this instead — rejected as the sole tool because graphify is better suited to one-off codebase/corpus graphs, while claude-obsidian is built for ongoing PKM with session memory, linting, and incremental ingestion; decided to run both, scoped differently (claude-obsidian = main PKM, graphify = one-off code/doc corpus graphs). Cloning a fresh vault per the plugin's quick-start — rejected, Sean already had a real vault with real notes (`Claude brain/`, `ICP/`) he didn't want to abandon.

**Owner:** Sean. Required installing the standalone `claude-code` npm CLI first (`npm install -g @anthropic-ai/claude-code`) since plugin management isn't exposed inside the VSCode-extension session — worth remembering if plugin installs come up again. `connections.md` row 6 ("Meeting intelligence / notes") still says Obsidian is "under consideration" — should be updated to reflect it's now live.
